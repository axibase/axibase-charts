...
<a name="column-key"></a>[`column-{key}`](#column-key)| Change the name of column `{key}`.<br>Hide `column-{key}` with `column-{key} = null`.<br>**Examples**:<li>`column-entity = Entity`<li>`column-value = null`| [↗](https://apps.axibase.com/chartlab/77a78ad8)
...
<a name="format"></a>[`format`](#format)|Cell value [format](../../syntax/format-settings.md).<br>**Examples**:<li>`format = kilobytes`<li>`format = ####`|[↗](https://apps.axibase.com/chartlab/95bd95be/8/)
...
<a name="style"></a>[`style`](#style)|Style assigned to the column.<br>Can be specified as JavaScript code.<br>**Examples**:<li>`style = text-align: left`<li>`style = background: orange`|[↗](https://apps.axibase.com/chartlab/95bd95be/23/)
...
<a name="on-click"></a>[`on-click`](#on-click)|JavaScript code click event handler for each cell.<br>Supports `method` filter, which allows you to filter the rows in the table to the value of this cell.<br>**Examples**:<li>`onclick = var s = series({ metric: 'nmon.process.%cpu', disconnectInterval: '90 second' })`<li>`onclick = dialog({ series: [s], disconnectValue: 0 })`<li>`onclick = filter()`|[↗](https://apps.axibase.com/chartlab/95bd95be/15/)
