...
<a name="fill-value"></a>[`fill-value`](#fill-value) | Interpolate a series with irregularly spaced values.<br>When `fill-value = true`, detailed values are filled using **linear** interpolation and aggregated periods are filled with **previous** values.<br>When `fill-value = false`, missing samples are filled with `0` or `null`.<br>Possible values: `false`, `true`.<br>Default value: `true`.<br>**Example**: `fill-value = true`| [↗](https://apps.axibase.com/chartlab/061698b1)