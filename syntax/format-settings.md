...
`decimal`| `format = numeric(1)`<br>`format = numeric`| Number formatting<br>`numeric` is an `alias` for `decimal`.<br>Specify the optional value parameter in round brackets to define how many spaces the decimal is rounded.<br>Example: `format = numeric(1)` rounds to the first decimal. Thus, `4.29` becomes `4.3`.
